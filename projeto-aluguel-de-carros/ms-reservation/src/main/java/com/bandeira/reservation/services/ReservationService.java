...
public class ReservationService {
    ...
    public String makeReservation(ReservationRequest reservaRequest) throws JsonProcessingException {
        ...
        Car car = carroFeign.findByCar(reservaRequest.car());

        if(car == null){
            throw new CarroNotFoundException();
        }

        var days = ChronoUnit.DAYS.between(reservaRequest.withdrawal()
                , reservaRequest.devolution());

        Reservation reservation = new Reservation(
                client.getEmail(),
                car.getName(),
                reservaRequest.withdrawal(),
                reservaRequest.devolution(),
                StatusReservation.AWAITING_PAYMENT,
                car.getDailyPrice().multiply(BigDecimal.valueOf(days)),
                LocalDateTime.now());
        ...
        if(coupon != null){
            var newValue = reservation.getValue().multiply(BigDecimal.valueOf(coupon.getDiscountPercentage()));
